...
class VehicleData:
    ...
    def update(self):
        ...
        for key in self.data:
            self.data[key]\
                .append(self.data[key][-1]+self.data[key][-1]*random.uniform(-0.0001, 0.0001))


DATA = VehicleData()
APP = dash.Dash('vehicle-data')
APP.layout = html.Div([html.Div([html.H2('Vehicle Data', style={'float':'left'})])\
    , dcc.Dropdown(id='vehicle-data-name', options=[{'label': s, 'value':s} for s in DATA.data]\
...
def update_graph(data_names, _):
    ...
    for names in data_names:
        ...
        graphs.append(html.Div(dcc.Graph(id=names, animate=True\
            , figure={'data':[data]\
                , 'layout': go.Layout(xaxis=dict(range=[min(DATA.times), max(DATA.times)])\
...
EXT_CSS = ['https://cdnjs.cloudflare.com/ajax/libs/materialize/0.100.2/css/materialize.min.css']
for css in EXT_CSS:
    APP.css.append_css({'external_url': css})

EXT_JS = ['https://cdnjs.cloudflare.com/ajax/libs/materialize/0.100.2/js/materialize.min.js']
for js in EXT_JS:
